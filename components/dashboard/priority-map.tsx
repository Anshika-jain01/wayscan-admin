...
export default function PriorityMap({ data, selectedId, onMarkerClick }: PriorityMapProps) {
  ...
  const mapCenter: [number, number] = selectedItem ? [selectedItem.lat, selectedItem.lng] : DEFAULT_CENTER;
    ...
    return L.divIcon({
      ...
      html: `
        <div class="relative flex items-center justify-center">
          ...
          <div class="relative w-${size/4} h-${size/4} rounded-full border-4 border-white shadow-xl flex items-center justify-center transition-all duration-300 transform ${isSelected ? 'scale-125' : 'hover:scale-110'}" style="background-color: ${color}; width: ${size}px; height: ${size}px;">
  ...
  return (
    <div className="w-full h-full relative group">
      ...
      >
        ...
        {selectedItem && <MapController center={[selectedItem.lat, selectedItem.lng]} />}
